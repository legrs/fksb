...
constexpr float MUSHI = 0.2f; //目標値付近で「無」になるため 6°ぐらい
constexpr float MUSHI_AVEL = 0; //目標値付近で「無」になるため 6°ぐらい
...
Vec avel_prev; // angular velocity = gyro
Vec acce;
Vec error_i(0,0,0);
...
void majiKeisandekiru3(){
    ...
    // error
    Vec oriv = ori.toV();
    Vec avel_ap = avel * LPF_FAC + avel_prev * LPF_FAC2;
    //Vec avel_ap = avel;
...
2      : フライトモード   0b01        10fps   1pps    true      false      打ち上げ待機モード この状態でZ軸向き加速度を検知するとmode4に移行
3      : 飛行中           0b10        30fps   3pps    true      false      飛行中のフラグ
