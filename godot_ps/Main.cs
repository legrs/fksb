...
public partial class Main : G.Control {
    ...
    public static float Kp = -0.7F*gain; 
    public static float Kd = -1F*gain;
    public static float Ki = -0.0F*gain;
    public static Vec3 target = new Vec3(0F,0,0);
    ...
    static float maxwav = 300; // rad/s
    //static float maxtorque = 10000000; 
    static float maxtorque = 16650000; //11.1W 1.5A 100rad/sのとき
    const float eps = 11.1F*3*1000000000;
    ...
    public override void _Ready(){
        ...
        // setup
        ori[0] = new Quat(new Vec3(1,0,0) , 1.0F);
    ...
    public override void _Process(double delta) {
        ...
        */
        ...
        rb.ApplyTorque(Vec3.add(
                    torque ,
                    Vec3.multiply( Vec3.cross( avel , wav ) , wmoi2-wmoi1 )
                    ).rotate(ori[0]).toGV());
    ...
    private void majiKeisandekiru3(double delta){
        ...
        error = Vec3.add(oriv[0] , target.invert());
        integral.add(error);
        ...
        torque.z = cmoi.z*deltaV.z/(float)delta + (cmoi.y - cmoi.x)*avel.y*avel.x;
            wav.add(maxwav/2);
        torque.clip(new Vec3(eps/wav.x , eps/wav.y , eps/wav.z));
        // true wav value area
            wav.add(-maxwav/2);
        wav.add(Vec3.multiply(torque , (float)delta/wmoi2));
        // to avoid saturation
        wav.add(Vec3.divide(wav , -100));
