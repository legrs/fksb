...
public partial class Canvas : Control
{
    ...
    public override void _Draw(){
        if(tc.CurrentTab == 0){
            ...
            Vector2 pos = Main.svcc.Position + Main.svcc.Size/2;
    
            DrawLine(pos, pos + xp, Main.red, 2);
            DrawLine(pos, pos + yp, Main.green, 2);
            DrawLine(pos, pos + zp, Main.blue, 2);
            makeArrowHead(arrowHead, pos, xp);
            DrawPolygon(arrowHead, [Main.red]);
            makeArrowHead(arrowHead, pos, yp);
            DrawPolygon(arrowHead, [Main.green]);
            makeArrowHead(arrowHead, pos, zp);
            DrawPolygon(arrowHead, [Main.blue]);
            
            q = new Quat(new Vec3(0,1,0) , camAngl[1]);
            q = Quat.concat(q,new Quat(new Vec3(0,0,-1), camAngl[0]));
    
            x = (new Main.Vec3(len,0,0)).rotate(q);
            y = (new Main.Vec3(0,len,0)).rotate(q);
            z = (new Main.Vec3(0,0,len)).rotate(q);
    
            xp = new Vector2(x.y , -x.z);
            yp = new Vector2(y.y , -y.z);
            zp = new Vector2(z.y , -z.z);
            pos = Main.svcc.Position + Main.svcc.Size - new Vector2(len+10,len+10);
            ...
            // dummy
            Main.graphDat[0].Add(acce.z/100);
            Main.graphDat[1].Add(acce.y/100);
            Main.graphDat[2].Add(acce.x/100);
            Main.graphDat[3].Add((float)index/255);
            ...
            for(int i=0; i<4; i++){
                if(2 < Main.graphDat[i].Count){
                    ...
                    for(int j=0; j<Main.graphDat[i].Count; j++){
                        ...
                        points[j].Y = -Main.graphDat[i][j]*Main.graphArea.Size.Y/2 + Main.graphArea.Size.Y/2 + Main.graphArea.Position.Y;
                    }
