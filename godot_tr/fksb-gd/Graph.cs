...
/*
    ...
    No longer used   please reference Canvas.cs
...
public partial class Graph : ColorRect
{
    ...
    public override void _Draw(){
        ...
        Vector2 xp = new Vector2(x.x , x.y);
        Vector2 yp = new Vector2(y.x , y.y);
        Vector2 zp = new Vector2(z.x , z.y);

        DrawLine(Size/2,Size/2 + xp, Main.red, 3);




        Main.graphDat[0].Add(MathF.Cos((float)time)*this.Size.Y/2);

        vec1.X = 0;
        vec1.Y = this.Size.Y/2;
        ...
        for(int i=0; i<4; i++){
            if(2 < Main.graphDat[i].Count){
                ...
                for(int j=0; j<Main.graphDat[i].Count; j++){
                    points[j].X = j*this.Size.X/Main.graphDatLen;
                    points[j].Y = -Main.graphDat[i][j] + this.Size.Y/2;
                }
